...
# MAGIC interval. About 2.8 million rows over a year.
# MAGIC
...
COMMENTS = {
    ...
    "quantity_mw": "Output as published. Negative is real and means the unit was "
                   "consuming, which is what pumped storage does.",
...
spark.sql(
    ...
    "one row per unit per production type per interval. A year of history, "
    "deliberately deeper than the seventy day market window, because saying a "
    "unit produced less than usual needs a baseline.'"
)
...
print("production types by output:")
(
...
# MAGIC duplicate key here would mean a unit reported twice for the same interval
# MAGIC and production type, which is possible in a republication and is not
# MAGIC something to crash on without looking.
...
nulls = table.select(
    [
        ...
        for column in ["zone", "unit_eic", "ts_utc", "quantity_mw", "resolution_minutes"]
    ]
).collect()[0].asDict()
...
duplicates = (
    table.groupBy("zone", "unit_eic", "psr_type", "ts_utc")
    .count()
    .filter(F.col("count") > 1)
)
duplicate_count = duplicates.count()
print(f"\nduplicate keys (zone, unit, type, timestamp): {duplicate_count:,}")
if duplicate_count:
